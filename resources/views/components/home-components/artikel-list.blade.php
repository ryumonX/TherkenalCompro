@props(['artikel' => [], 'kategori' => [], 'artikelTerbaru' => []])
  ...
  <div class="container mx-auto px-4 lg:px-6 relative z-10">
    ...
    <div class="relative mb-12" id="kategori-container">
      ...
      <div class="hidden md:flex justify-center gap-2 flex-wrap" id="desktop-kategori-static">
        <a href="{{ route('artikel.index') }}" class="py-2 px-4 rounded-full {{ !request('kategori') ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }} font-medium hover:bg-blue-700 hover:text-white transition-all">
          Semua Artikel
        </a>

        @foreach($kategori as $kat)
          <a href="{{ route('artikel.index', ['kategori' => $kat->slug]) }}" class="py-2 px-4 rounded-full {{ request('kategori') == $kat->slug ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }} font-medium hover:bg-blue-700 hover:text-white transition-all">
            {{ $kat->title }}
          </a>
        @endforeach
      </div>
      ...
      <div class="swiper kategori-swiper overflow-hidden hidden" id="kategori-swiper">
        <div class="swiper-wrapper">
          ...
          <div class="swiper-slide w-auto">
            <a href="{{ route('artikel.index') }}" class="py-2 px-4 rounded-full {{ !request('kategori') ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }} font-medium hover:bg-blue-700 hover:text-white transition-all inline-block whitespace-nowrap">
              Semua Artikel
            </a>
          </div>
          ...
          @foreach($kategori as $kat)
            <div class="swiper-slide w-auto">
              <a href="{{ route('artikel.index', ['kategori' => $kat->slug]) }}" class="py-2 px-4 rounded-full {{ request('kategori') == $kat->slug ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700' }} font-medium hover:bg-blue-700 hover:text-white transition-all inline-block whitespace-nowrap">
                {{ $kat->title }}
              </a>
            </div>
          @endforeach
    ...
    <div class="bg-blue-50 rounded-lg p-4 mb-8 flex items-center justify-between">
      ...
      <a href="{{ route('artikel.index') }}" class="text-blue-600 hover:text-blue-800 flex items-center text-sm">
    ...
    <div class="flex flex-col lg:flex-row gap-8 lg:gap-12">
      ...
      <div class="lg:w-2/3">
        ...
        @if(isset($artikel) && !is_array($artikel) && method_exists($artikel, 'hasPages'))
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    const kategoriCount = {{ count($kategori) }} + 1; // +1 untuk "Semua Artikel"
